...
public class IngredienteService {
    ...
    public List<IngredienteDTO> getAll() {
        return repo.findAll().stream()
                .map(i -> modelMapper.map(i, IngredienteDTO.class))
    ...
    public int updateById(Integer id, IngredienteDTO newData) {
        ...
        if (newData.getEstado() != null) i.setEstado(newData.getEstado());
        if (newData.getCostoUnitario() != null) i.setCostoUnitario(newData.getCostoUnitario());
        repo.save(i);
        return 0;
    }
