...
public class InventarioService {
    ...
    public int updateById(Integer id, InventarioDTO newData) {
        ...
        if (newData.getStockMinimo() != null) inv.setStockMinimo(newData.getStockMinimo());

        if (newData.getAdministradorId() != null) {
            Administrador a = adminRepo.findById(newData.getAdministradorId())
                    .orElseThrow(() -> new RuntimeException("Administrador no existe: " + newData.getAdministradorId()));
            inv.setAdministrador(a);
        } else {
            inv.setAdministrador(null);
        }
