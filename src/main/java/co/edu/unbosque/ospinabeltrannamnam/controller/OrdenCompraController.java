...
public class OrdenCompraController {
	...
	public ResponseEntity<String> crear(@RequestBody OrdenCompraDTO dto) {
		...
		if (res != 0)
			return ResponseEntity.badRequest().body("No se pudo crear la orden de compra.");
	...
	public ResponseEntity<?> obtener(@PathVariable Integer id) {
		...
		if (dto == null)
			return ResponseEntity.badRequest().body("Orden de compra no encontrada.");
	...
	public ResponseEntity<String> eliminar(@PathVariable Integer id) {
		...
		if (res != 0)
			return ResponseEntity.badRequest().body("No existe la orden de compra especificada.");
