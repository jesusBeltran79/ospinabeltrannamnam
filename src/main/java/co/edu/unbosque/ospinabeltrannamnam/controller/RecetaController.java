...
public class RecetaController {
	...
	public ResponseEntity<String> crear(@RequestBody RecetaDTO dto) {
		...
		if (res != 0)
			return ResponseEntity.badRequest().body("No se pudo crear la receta. Verifique los datos.");
	...
	public ResponseEntity<String> eliminar(@PathVariable Integer id) {
		...
		if (res != 0)
			return ResponseEntity.badRequest().body("No existe la receta especificada.");
